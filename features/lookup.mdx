...
<Tip>
If a street is not found, try a shorter substring. For example, if `AV LEHENDAKARI AGUIRRE` returns nothing, try `LEHENDAKARI AGUIR` — that is the key used in the base dictionary for that street.
</Tip>
